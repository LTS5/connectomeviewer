...
def visualize_graph(nodeids, labels, edges, scalars, \
    ...
    for i in actors_list:
        ...
        # SEE hint by gael to scale glyphs!
...
def vis_VTK():
    ...
    G = tvtk.MutableUndirectedGraph()
    G.add_vertex()
    G.add_vertex()
    G.add_vertex()
    G.add_graph_edge(0,1)
    G.add_graph_edge(1,2)
...
def vis_G():
    ...
    mlab.figure()
    #ls = mlab.pipeline.line_source(dl([0,1,2,3]), dl([3,2,4,3]), dl([4,5,3,2]), dl([1,2,3,4]))
    ls = mlab.pipeline.line_source([0, 0, 2, 2, 2, 3, 3, 3], \
                [3, 3, 2, 2, 4, 4, 3, 3],\
                [4, 4, 5, 5, 3, 3, 2, 2],\
                [0, 0.1, 0.1, 0.8, 0.8, 0.4, 0.4, 0.9] )
    tube = mlab.pipeline.tube(ls, tube_radius= 0.05, tube_sides=10)
    surf = mlab.pipeline.surface(tube)
    
    #mlab.plot3d([0.00001,0,1,2,3,3.00001],[3.00001,3,0,2,1,1.00001],[2.00001,2,0,3,1,1.00001],[2, 2,3,3,4, 4], tube_radius=0.08)
    
    #
    #mlab.plot3d([0, 0, 2, 2, 2, 3, 3, 3], \
    #            [3, 3, 2, 2, 4, 4, 3, 3],\
    #            [4, 4, 5, 5, 3, 3, 2, 2],\
    #            [0, 2, 2, 2, 2, 3, 3, 4])
    
    #mlab.plot3d()
    #using plot3d to color edges
