...
vtkCxxRevisionMacro(vtkEdgeTubeFilter, "$Revision: 1.72 $");
vtkStandardNewMacro(vtkEdgeTubeFilter);

// Construct object with radius 0.5, radius variation turned off, the number 
// of sides set to 3, and radius factor of 10.
vtkEdgeTubeFilter::vtkEdgeTubeFilter()
{
  this->Radius = 0.5;
...
void vtkEdgeTubeFilter::Execute()
{
  ...
  vtkDataArray *inScalars=cd->GetScalars();
  vtkDataArray *inVectors=cd->GetVectors();
  ...
  for (inCellId=0, inLines->InitTraversal(); 
    ...
    //
    if ( !this->GeneratePoints(inCellId,offset,npts,pts,inPts,newPts,pd,outPD,
    ...
    if ( newTCoords )
      {
      this->GenerateTextureCoords(inCellId,offset,npts,pts,inPts,inScalars,newTCoords);
      }
...
int vtkEdgeTubeFilter::GeneratePoints(
																	vtkIdType cellId,
																	vtkIdType offset, 
  ...
  for (j=0; j < npts; j++)
    ...
    if ( inScalars && this->VaryRadius == VTK_VARY_RADIUS_BY_SCALAR )
      ...
      sFactor = 1.0 + ((this->RadiusFactor - 1.0) * 
                (inScalars->GetComponent(cellId,0) - range[0]) 
                       / (range[1]-range[0]));
      }
    else if ( inVectors && this->VaryRadius == VTK_VARY_RADIUS_BY_VECTOR )
      {
      sFactor = 
        sqrt((double)maxSpeed/vtkMath::Norm(inVectors->GetTuple(cellId)));
...
void vtkEdgeTubeFilter::GenerateStrips(vtkIdType offset, vtkIdType npts, 
...
void vtkEdgeTubeFilter::GenerateTextureCoords(
																					vtkIdType inCellId,
																					vtkIdType offset,
                                          vtkIdType npts, vtkIdType *pts, 
                                          vtkPoints *inPts, 
                                          ...
                                          vtkFloatArray *newTCoords)
{
  ...
  if ( this->GenerateTCoords == VTK_TCOORDS_FROM_SCALARS )
    {
    s0 = inScalars->GetTuple1(inCellId);
    for (i=1; i < npts; i++)
      {
      s = inScalars->GetTuple1(inCellId);
...
vtkIdType vtkEdgeTubeFilter::ComputeOffset(vtkIdType offset, vtkIdType npts)
{
  if ( this->SidesShareVertices )
    {
...
const char *vtkEdgeTubeFilter::GetVaryRadiusAsString(void)
{
...
const char *vtkEdgeTubeFilter::GetGenerateTCoordsAsString(void)
{
...
void vtkEdgeTubeFilter::PrintSelf(ostream& os, vtkIndent indent)
{
