...
if sys.platform == 'linux2':
    ...
    if '32' in pf.architecture()[0]:
        giftilib = CDLL(join( dirname(__file__), "linux2", "libgiftiio.so"))
    elif '64' in pf.architecture()[0]:
        giftilib = CDLL(join( dirname(__file__), "linux2", "libgiftiio64.so"))
    else:
        raise('libgiftiio.so not compiled for your platform. Please contact the developers!')
...
class GiftiDataArray(Structure):
    _fields_ = [("intent", c_int),
                ...
                ("meta", GiftiMetaData),
                ("coordsys", POINTER(POINTER((GiftiCoordSystem)))),
                ("data", c_void_p),
                ("nvals", c_longlong),
                ("nbyper", c_int),
                ("numCS", c_int),
...
def gifti_add_empty_CS(da):
    return giftilib.gifti_add_empty_CS(pointer(da))
...
def gifticlib_version():
    giftilib.gifticlib_version.restype = c_char_p
    return giftilib.gifticlib_version()
