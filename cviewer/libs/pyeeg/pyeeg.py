...
PyEEG, a Python module to extract EEG features, v 0.01_r4

Project homepage: http://pyeeg.org

**Data structure**

pyeeg only uses standard Python and numpy data structure,
...
#		if max(abs(Em[N-M]-Em[N-M])) <= R: # index from 0, so N-M+1 is N-M  v 0.01b_r1
	...
	Phi_m, Phi_mp = sum(log(Cm)) / (N - M + 1),  sum(log(Cmp)) / (N - M )
